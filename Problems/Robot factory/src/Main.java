...
class RobotFactory {
    ...
    public Robot getRobot(RobotType type, String name, String description, int power) {
        if (type == RobotType.ROBOT_CLEANER) return new RobotCleaner(name, description, power);
        if (type == RobotType.ROBOT_GUARDIAN) return new RobotGuardian(name, description, power);
        return null;
    }
}
