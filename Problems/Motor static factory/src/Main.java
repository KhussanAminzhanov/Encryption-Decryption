...
class MotorStaticFactory {
    ...
    public static Motor make(char type, String model, long power) {
        if (type >= 65 && type <= 90) type = (char) (type + 32);
        if (type == 'p') return new PneumaticMotor(model, power);
        else if (type == 'h') return new HydraulicMotor(model, power);
        else if (type == 'e') return new ElectricMotor(model, power);
        else if (type == 'w') return new WarpDrive(model, power);
        else return null;
    }
}
